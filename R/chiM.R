...
chiM_iter <- function(predictors, labels, test = F, validation = F, proportions = c(0.3, 0.3), criterion = "gini", param = list(alpha = 0.05)) {
  ...
  if (validation == TRUE) {
    ...
    if (criterion == "gini") {
      best.disc <- list(logit[[which.min(ginidisc)]], disc[[which.min(ginidisc)]], which.min(ginidisc))
      if (test == TRUE) {
        data_validation <- as.data.frame(sapply(as.data.frame(discretize_cutp(predictors[ensemble[[2]], ], disc[[i]][["Disc.data"]], predictors[ensemble[[1]], ])), as.factor), stringsAsFactors = TRUE)
  ...
  if (test == TRUE) {
    return(methods::new(Class = "discretization", method.name = "chiM", parameters = list(predictors, test, validation, criterion, param, ensemble), best.disc = best.disc, performance = list(performance), disc.data = data.frame(cbind(discretize_cutp(predictors[ensemble[[3]], ], best.disc[[2]][["Disc.data"]], predictors[ensemble[[1]], ]), labels[ensemble[[3]]]), stringsAsFactors = TRUE), cont.data = data.frame(cbind(predictors[ensemble[[3]], ], labels[ensemble[[3]]]))))
