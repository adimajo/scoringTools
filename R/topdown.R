...
topdown_iter <- function(predictors,labels,test=TRUE,validation=TRUE,criterion='gini',param=list(1,2,3)) {
     if (criterion %in% c('gini','aic')) {
          if (length(labels)==length(predictors[,1])) {
               ...
               if (test==TRUE) {
                    return(methods::new(Class = "discretization", method.name = "topdown", parameters = list(predictors,test,validation,criterion,param), best.disc = best.disc, performance = list(performance), disc.data = data.frame(cbind(discretize_link(best.disc[[2]],predictors[ensemble[[3]],]),labels[ensemble[[3]]])), cont.data = data.frame(cbind(predictors[ensemble[[3]],],labels[ensemble[[3]]]))))
