context("test-parcelling")

test_that("parcelling method works with speedglm and glm", {
  ...
  modele_rejected <- parcelling(xf, xnf, yf)
  expect_s4_class(modele_rejected, "reject_infered")
  expect_equal(modele_rejected@method_name, "parceling")
  ...
  with_mock(
    ...
    {
      expect_warning(modele_rejected <- parcelling(xf, xnf, yf))
      expect_s4_class(modele_rejected, "reject_infered")
      expect_equal(modele_rejected@method_name, "parceling")
      expect_s3_class(modele_rejected@financed_model, "glmORlogicalORspeedglm")
      expect_true(is.na(modele_rejected@acceptance_model))
