...
test_that("chiM method works with speedglm and glm on matrices", {
...
test_that("chiM method works with speedglm and glm on dataframes with additional factors", {
  x <- matrix(runif(140), nrow = 70, ncol = 2)
  cuts <- seq(0, 1, length.out = 4)
  xd <- apply(x, 2, function(col) as.numeric(cut(col, cuts)))
  ...
  for (test in c(TRUE, FALSE)) {
    for (validation in c(TRUE, FALSE)) {
      for (criterion in c("gini", "aic")) {
        chiM_modele <- chiM_iter(data.frame(x, factor(xd[, 1])), y, test = test, validation = validation, criterion = criterion, param = list(alpha = 0.01))
        expect_s4_class(chiM_modele, "discretization")
        expect_equal(chiM_modele@method.name, "chiM")
        expect_equal(
          chiM_modele@parameters[-length(chiM_modele@parameters)],
