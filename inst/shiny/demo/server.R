server <- function(input, output, session) {
  ...
  shiny::observe({
    if (input$use_MNAR) {
      list_imported_df[["MNAR"]] <- scoringTools::generate_data(n = input$n_MNAR,
                                                                d = input$dim_MNAR,
                                                                type = "MNAR")
    } else {
      tryCatch(
  ...
  model_reject_inference <- reactive({
    ...
    for (model in (input$reject)) {
      switch(
        model,
        augmentation = {
          list_models[[model]] <- scoringTools::augmentation(x_f_train,
                                                             x_nf_train,
                                                             y_f_train)
          roc_curves[[model]] <- pROC::roc(c(y_f_test, y_nf_test),
                                           predict(list_models[[model]],
                                                   rbind(x_f_test, x_nf_test),
                                                   type = "response"))
          roc_curves_financed[[model]] <- pROC::roc(y_f_test,
                                                    predict(list_models[[model]],
                                                            x_f_test,
                                                            type = "response"))
          # list_gini[[model]] <- scoringTools::normalizedGini(
